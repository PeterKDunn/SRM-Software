# Using jamovi for inference about ORs  {#jamoviChisquare}
...
* How to use jamovi for conducting chi-square tests
...
$\chi^2$ tests (*ki* as in 'kite') are used to analyse data where each unit of analysis has
...
**This data set is available from the textbook as `B12`** in 
...
So jamovi will have *two columns*: 
one for each variable.


There are 124 units of analysis,
so there are *124 rows*.
...
See Fig. \@ref(fig:jamoviChisqB12DataLong).

The data needs to be set up so that
*each row represents one unit of analysis*.
That means there will be 124 rows of data: 
one for each person (unit of analysis).

In jamovi,
the data would be entered as shown in
Fig. \@ref(fig:jamoviChisqB12DataLong);
there would be 124 rows, but we only show some of these rows to save space.
...
Remember that **this data set is available from the textbook as `B12`** in 
[Appendix&nbsp;A](https://bookdown.org/pkaldunn/SRM-Textbook/AppendixDataSets.html)
...
2. **Select the variables**,
   ...
   **Rows** and **Columns** areas as appropriate.
   In Table \@ref(tab:jamoviB12Data),
...
3. **Select the analysis options.**
   Under the area where you entered the variables,
   select the right-pointing arrow beside **Statistics**,
   and then:

    - Select **$\chi^2$** (to produce information for the hypothesis test);
    - Select **Confidence intervals** (to produce a CI for the odds ratio).
...
*Percentages* can be obtained from the same place: 
**Analyses: Frequencies Independent samples ($\chi^2$ test of association)**.
Under the area where you selected the **Statistics**,
select the right-pointing arrow beside **Cells**,
and then:
