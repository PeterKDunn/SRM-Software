...
5. You can change the vertical axis to represent either counts or percentages by your selection under **Y-Axis**.
6. You can change which variables is on the horizontal axis by your selection under **X-Axis**.
   There are plenty of options, so feel free to experiment. 
   A useful one is to use for the **Y-Axis** is `Percentages` with the `within rows` option.
...
1. **Use the jamovi menu:** select **Analyses> Exploration> Descriptives**.
2. **Add the variables**.
   Place the quantitative variable in the *Variables* area, and the qualitative variable in the **Split by** area.
3. Then, click on the right-pointing arrow next to **Plots**, and in the **Box Plots** area select `Box plot`.
...
At the time these notes were prepared, *jamovi does not allow graphs to be edited* (but it is promised to be coming soon!).
